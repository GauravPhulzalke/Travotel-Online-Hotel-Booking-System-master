    				...
    				if(isset($_GET['searched']))	
					{
						...
						while($row = mysqli_fetch_array($res))
						{
							if (substr_compare($searched, $row['hloc'], 0, 5, true)==0) 
							{
								$hname = $row['hname'];
								$hloc = $row['hloc'];
								echo '<form method = "get" action = "http://localhost/Forms/Reservation.php?searched='.$hname.'">';
								
								echo '<input type="hidden" name="hloc" value="'.$hloc.'">';


								echo '<img class="column 4" src= "pics/'.$row['imgname'].'" width=300px height=auto align=left>'.'<br>';
